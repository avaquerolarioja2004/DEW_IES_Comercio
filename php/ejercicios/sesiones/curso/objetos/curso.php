...
class Curso
{
    private $cursoDir = 'V:\\DWES_DAW2\\Ampps\\www\\dwes1\\php\\ejercicios\\sesiones\\curso\\cursos';
    private $totalPaginas = 100;

    public function generarCurso()
    {
        for ($i = 1; $i <= $this->totalPaginas; $i++) {
            $pagina = $this->cursoDir . "/pagina" . $i . ".php";

            // Solo crea la página si no existe
            if (!file_exists($pagina)) {
                $contenido = $this->contenidoPagina($i);
                file_put_contents($pagina, $contenido);
                echo "Página $i creada correctamente.<br>";
            }
        }
    }

    private function contenidoPagina($pagina)
    {
        return "<?php
        session_start();
        ...
        \$pagina = 'pagina' . $pagina . '.php';  // La página actual
        ...
        actualizarUltimaPagina(\$usuario, $pagina, \$usuarios, \$usuarioFile);

        echo '<h2>Bienvenido a la página $pagina del curso</h2>';
        echo '<a href=\"../logout.php\">Cerrar sesión</a>';

        // Enlaces de navegación
        echo '<p>';
        if ($pagina > 1) {
            echo '<a href=\"pagina' . ($pagina - 1) . '.php\">Página anterior</a>';
        }
        if ($pagina < 100) {
            echo ' | <a href=\"pagina' . ($pagina + 1) . '.php\">Página siguiente</a>';
        }
        echo '</p>';
...
// Crear una instancia de la clase Curso y generar las páginas
$curso = new Curso();
$curso->generarCurso();
?>
